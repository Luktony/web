          ...
          summary.map((task) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <Dialog.Root open={isEditing === task.id} onOpenChange={(open) => !open && handleCloseEdit()}>
                  <Dialog.Trigger type="button" onClick={() => handleEditClick(task.id)}>
                    <Pencil size={23} />
                  </Dialog.Trigger>
                  <Dialog.Portal>
                    <Dialog.Overlay />
                    <Dialog.Content className="absolute p-10 bg-gray-800 rounded-lg w-11/12 max-w-lg top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      ...
                      <EditTask data={task.id} onClose={handleCloseEdit} />
                    </Dialog.Content>
                  </Dialog.Portal>
                </Dialog.Root>
